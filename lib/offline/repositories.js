...
export const OrderRepository = {
    ...
    if (IS_POWERSYNC_WRITES_ENABLED && typeof window !== "undefined") {
      try {
        ...
        if (psDb) {
          await psDb.init();
          await psDb.writeTransaction(async (tx) => {
            await tx.execute("UPDATE order_items SET status = ? WHERE id = ?", [newStatus, itemId]);
            const allItems = await tx.getAll("SELECT * FROM order_items WHERE order_id = ?", [orderId]);
            const allDone = allItems.every((row) => (row.id === itemId ? newStatus === "done" : row.status === "done"));
            if (allDone) {
              await tx.execute("UPDATE orders SET status = 'done' WHERE id = ?", [orderId]);
            } else {
              await tx.execute("UPDATE orders SET status = 'preparing' WHERE id = ?", [orderId]);
            }
          });
        ...
        console.warn("[OrderRepository] PowerSync item status write failed, falling back to IndexedDB write:", err);
      }
    }
  ...
  async markOrderBilled(orderId, tableNumber) {
    if (IS_POWERSYNC_WRITES_ENABLED && typeof window !== "undefined") {
      try {
        const psDb = getProductionPowerSyncDB();
        if (psDb) {
          await psDb.init();
          await psDb.writeTransaction(async (tx) => {
            await tx.execute("UPDATE orders SET status = 'billed' WHERE id = ?", [orderId]);
            if (tableNumber) {
              await tx.execute("UPDATE tables SET status = 'Available', current_order_id = NULL WHERE number = ?", [Number(tableNumber)]);
            }
          });
        ...
        console.warn("[OrderRepository] PowerSync bill write failed, falling back to IndexedDB write:", err);
      }
    }
    ...
    await new Promise((resolve, reject) => {
      const tx = db.transaction(["orders", "tables", "sync_outbox"], "readwrite");
      const orderStore = tx.objectStore("orders");
      const tableStore = tx.objectStore("tables");
    ...
    try {
      ...
      for (const task of pendingTasks) {
        try {
          ...
          } else if (task.action === "BILL_ORDER") {
            const res = await fetch("/api/orders", {
              method: "PATCH",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
